...
void clear_hash_table(Node* bucket[],int &stored_elements,int &collisions,int *buffer,int bucket_size) {
    stored_elements = 0;
    collisions = 0;
...
void display_hash_table(Node *bucket[],int bucket_size) {
...
void rehashing(Node** bucket,Node* pointer,int bucket_size,int &collisions,int buffer[]) {
    int index = 0;
    hash_function(pointer->Key,index,bucket_size);
    Node *Head = bucket[index];
    if (Head == nullptr) {
        Head = pointer;
    }
...
std::pair<Node**,int*> hash_resizing(int &bucket_size,Node **bucket,int &collisions) {
    collisions = 0;
    bucket_size*=2;
    int* buffer_resize = new int[bucket_size]();
    Node** bucket_resize = new Node*[bucket_size]();
    for (int i=0;i<bucket_size/2;i++) {
        Node *Head = bucket[i];
        while (Head) {
            Node *temp = Head->next;
            rehashing(bucket_resize,Head,bucket_size,collisions,buffer_resize);
            Head = temp;
        }
    }
    delete[] bucket;
    return {bucket_resize,buffer_resize};
}
int main() {
    int bucket_size = 8;
    int stored_elements = 0;
    int collisions = 0;
    int* buffer = new int[bucket_size]();
    Node** bucket = new Node*[bucket_size]();
    while (true) {
        if ((float)stored_elements/bucket_size >= 0.75) {
            std::pair<Node**,int*>pair = hash_resizing(bucket_size,bucket,collisions);
            delete[] buffer;
            bucket = pair.first;
            buffer = pair.second;
        }
        ...
        if (move == 6) {
            display_hash_table(bucket,bucket_size);
        }
        if (move == 7) {
            int longest_chain = 0;
        ...
        if (move == 10) {
            clear_hash_table(bucket,stored_elements,collisions,buffer,bucket_size);
